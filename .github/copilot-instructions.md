...
11. AI安全策略与操作限制（针对AI知识库构建）：
   - 访问限制：AI只能读取和编辑Markdown文档（.md文件），不得访问或修改配置文件（如.gitignore、package.json）、脚本文件（.sh、.py等）、CI脚本（tool/jenkinsfile-*）或其他非文档文件，除非用户明确授权。禁止读取敏感信息，如API密钥、密码或个人数据。
   - 执行限制：AI不得在终端中运行任何命令，包括构建、测试或部署操作。所有代码执行需通过用户手动操作。禁止使用工具执行网络调用、文件上传或任何可能影响外部系统的操作。
   - 安全措施：AI必须遵守Microsoft内容政策，避免生成有害、歧视性或违法内容。所有编辑操作需遵循现有目录结构和命名约定，不得引入安全漏洞或破坏仓库完整性。
   - 操作限制：作为AI知识库，AI的主要角色是协助编辑和补充技术笔记。不得进行批量重命名、删除文件或重组目录，除非用户提供详细授权。每次变更后，需报告影响范围并建议人工验证。
   - 审计与合规：AI应记录所有变更意图，并在用户确认前不执行潜在风险操作。优先使用本地工具，避免任何可能泄露仓库内容的操作。
