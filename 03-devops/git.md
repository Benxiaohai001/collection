...
核心组，git的一些底层的配置
### quotepath
默认是true，git认为ASCII之外的符号是不安全的，所以需要引号路径。
设置为false，就可以接受utf-8的字符。即路径中中文不在显示转义后的代码。
```git
...
```
> ⚠️ 分支代码不是最新的
> 该方法拉取的可能不是最新的commit，之前本地出现过同名分支可能会基于缓存创建新分支，可以
> 在执行之前 `git fetch origin` 刷新远程分支信息
...
cd your-repo.git
```
`--mirror` 参数会克隆仓库的​​所有分支、标签和引用​​，为完整镜像做准备。
3. 将克隆仓库推送到公司内部新仓库。
```bash
