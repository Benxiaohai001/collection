# 概念
## term（任期）
选举任期是单调递增的。任期号大的为准。（相当于皇帝，一任一任的）
## index（索引）
索引也是单调递增的，且全局单调递增，不会因为term重置。（相当于史书页码）
term+index是解决脑裂问题的核心参考。
## Quorum（多数派）
至少要达到的节点数量（n/2+1）
* 防止脑裂
* 容错边界：不足够多数派，会只读不写。
## commit（提交/确立）
三个阶段：提案=》确认-〉commited 相当于是将提案，变成了决议。
## log entry(日志)
term，index，command 三个部分，可以认为是raft的内容。
## state machine （状态机）
像一个黑盒子，表示为，通过日志获取一个结果。
行为上，通过日志进行计算；
表现上，通过计算得到的一个结果。
# 是什么？
