...
def lp_generator_thread(start_time, end_time, interval, ring_buffer):
    ...
    while start_time <= end_time:
        ...
        random_strings = [''.join(random.choices(string.ascii_letters + string.digits, k=random.randint(10, 40))) for _ in range(800)]
        # random_ints = [random.randint(1, 100000) for _ in range(150)]
        random_ints = [f"{random.randint(1, 100000)}i" for _ in range(600)]
        random_doubles = [round(random.uniform(1.0, 100000.0), 4) for _ in range(600)]
        combined_data = random_ints + random_doubles
        # tag = f'tag={random_strings[0]}'
        field_str_set = ",".join([f'field_str{i}="{random_strings[i]}"' for i in range(800)])
        field_set = ",".join([f"field{i}={combined_data[i]}" for i in range(1200)])
